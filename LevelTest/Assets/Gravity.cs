...
public class Gravity : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Vector3.Magnitude(pos_vec) <= 25) {
            if (circleOrbit) {
                v = Mathf.Sqrt((G * rb1.mass) / Vector3.Magnitude(pos_vec)) * (Vector3.Magnitude(pos_vec) * 5);
                rb2.velocity = v * Vector3.Normalize(new Vector3(-pos_vec.y, pos_vec.x, 0));
